...
import { deleteTask, getDone, getInProgress, getProgressbyId, getTodo, updateStatus } from './redux/slice';
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      <>
            ...
            {(provided) => (
              ...
              >
                ...
                <div className="tasks" data-status="todo">
                  {task && task.map((tasks, index) => (
                    <Draggable key={tasks._id} draggableId={tasks._id.toString()} index={index}>
                      {(provided, snapshot) => (
                        <div
                          className="task"
                          ref={provided.innerRef}
                        ...
                        >
                          ...
                          <button onClick={() => toggleEditModal(tasks._id)}>
                                <FontAwesomeIcon icon={faEdit} />
                              </button>
                            <button onClick={()=>{handleDelete(tasks._id)}}><FontAwesomeIcon icon={faTrash} /></button>
                          </div>
                        </div>
                      )}
                    </Draggable>
            ...
            {(provided) => (
              ...
              >
                ...
                <div className="tasks" data-status="in-progress">
                  {inProgressTask && inProgressTask.map((tasks, index) => (
                    <Draggable key={tasks._id} draggableId={tasks._id.toString()} index={index}>
                      {(provided, snapshot) => (
        ...
        {Editmodal && <Update toggleEditModal={toggleEditModal} taskId={selectedTask} />}
        {deletemodal && 
        <DeleteModal
